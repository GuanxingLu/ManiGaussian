...
class VisualizationDemo(object):
    ...
    def predict(self, original_image):
        ...
        # cprint("CPU Inference time: {:.2f}s".format(time.time() - start_time), "red")
        ...
        with torch.no_grad():
            ...
            self.model.cuda()
            predictions = self.model([inputs])[0]
            cprint("GPU Inference time: {:.2f}s".format(time.time() - start_time), "red")
        """
        for 1024x1024 img (short_edge_length=1024)
        CPU Inference time: 7.54s
        GPU Inference time: 1.43s

        for 512x512 img (short_edge_length=512)
        CPU Inference time: 2.10s
        GPU Inference time: 0.43s

        for 128x128 img (short_edge_length=128)
        CPU Inference time: 1.85s
        GPU Inference time: 0.41s
        """
        return predictions
...
if __name__ == "__main__":
    ...
    # change the short_edge_length to 512
    dataset_cfg.mapper.augmentations[0]['short_edge_length'] = img_size
    ...
    with ExitStack() as stack:
        ...
        if args.input:
            ...
            for path in tqdm.tqdm(args.input, disable=not args.output):
                ...
                cprint(f"inference time: {time.time() - inference_time:.2f}s", "cyan")
                logger.info(
                    "{}: {} in {:.2f}s".format(
                        path,
