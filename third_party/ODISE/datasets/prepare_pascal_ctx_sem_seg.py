...
def generate_labels(img_info, detail, out_dir):
    ...
    sem_seg = _class_to_index(detail.getMask(img_info), _mapping=_mapping, _key=_key)
...
if __name__ == "__main__":
    dataset_dir = Path(os.getenv("DETECTRON2_DATASETS", "local_data")) / "pascal_ctx_d2"
    ...
    for split in ["training", "validation"]:
        ...
        for img_info in tqdm.tqdm(img_infos):
            ...
            shutil.copy2(img_dir / img_info["file_name"], output_img_dir)
            generate_labels(img_info, detail_api, output_ann_dir)
