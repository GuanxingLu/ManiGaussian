...
class FeatureExtractorBackbone(Backbone):
    ...
    def __init__(
        ...
        if isinstance(backbone_in_size, int):
            self.image_preprocess = T.Resize(
                size=backbone_in_size, max_size=1280, interpolation=T.InterpolationMode.BICUBIC, antialias=True
            )
            self.backbone_in_size = (backbone_in_size, backbone_in_size)
            self._slide_inference = False
        else:
            self.image_preprocess = T.Resize(
                size=tuple(backbone_in_size), interpolation=T.InterpolationMode.BICUBIC, antialias=True
            )
    ...
    def extract_features(self, img):
        feature_dict = self.forward(img)
        return feature_dict['s2']
