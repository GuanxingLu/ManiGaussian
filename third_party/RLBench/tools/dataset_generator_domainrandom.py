...
from rlbench import VisualRandomizationConfig, RandomizeEvery
...
flags.DEFINE_float('train_ratio', 0.7,
                  'The ratio of training variations.')
flags.DEFINE_string('mode', 'train', 
            'train or test')
flags.DEFINE_bool('use_domain_randomization', True, 
            'domain randomization or not')
...
def run_all_variations(i, lock, task_index, variation_count, results, file_lock, tasks):
    ...
    if FLAGS.use_domain_randomization:
        ...
        print(colored('[run_all_variations] Using domain randomization', 'green'))
    else:
        rand_config = None
        randomize_every = None
    ...
    while True:
        ...
        # split the variation into train and test
        train_ratio = FLAGS.train_ratio
        train_variations = int(possible_variations * train_ratio)
        train_variation_list = list(range(train_variations))
        test_variation_list = list(range(train_variations, possible_variations))
        if FLAGS.mode == 'train':
            variation_list = train_variation_list
        elif FLAGS.mode == 'test':
            variation_list = test_variation_list
        else:   
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                try:
                    ...
                    variation = np.random.choice(variation_list)
                    task_env = rlbench_env.get_task(t)
