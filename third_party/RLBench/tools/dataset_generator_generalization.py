...
def run_all_variations(i, lock, task_index, variation_count, results, file_lock, tasks):
    ...
    while True:
        ...
        # split variations into train and test
        num_train_variation = int(possible_variations * 0.7)
        num_test_variation = possible_variations - num_train_variation
        # make the number list
        all_variation_numbers = np.arange(possible_variations)
        train_variation_numbers = all_variation_numbers[:num_train_variation]
        test_variation_numbers = all_variation_numbers[num_train_variation:]
        print(colored('Task: %s, Train variations: %d, Test variations: %d' % (t, num_train_variation, num_test_variation), 'cyan'))
        print(colored(f'Current mode: {FLAGS.mode}', 'cyan'))
        ...
        for ex_idx in range(FLAGS.episodes_per_task):
            ...
            while attempts > 0:
                try:
                    if FLAGS.mode == 'train':
                        variation = train_variation_numbers[ex_idx % num_train_variation]
                    elif FLAGS.mode == 'test':
                        variation = test_variation_numbers[ex_idx % num_test_variation]
                    else:
